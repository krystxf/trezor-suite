...
interface CoinmarketUseComposeTransactionProps<T extends CoinmarketSellExchangeFormProps> {
    ...
    inputAmountName: typeof FORM_CRYPTO_INPUT | typeof FORM_OUTPUT_AMOUNT;
}
    ...
    useEffect(() => {
        ...
        if (composed.type === 'final') {
            if (typeof setMaxOutputId === 'number' && composed.max) {
                setValue(inputAmountName, composed.max, {
                    shouldValidate: true,
                    shouldDirty: true,
                });
                clearErrors(inputAmountName);
            }
