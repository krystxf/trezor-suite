...
describe('keys', () => {
    ...
    it('AESGCM encode/decode', () => {
        const key = Buffer.from(
            'ccbf529fc8dd4662d4d1d1fa66368b8758c0b6673a1bb9d532d95ca607cbf729',
            'hex',
        ); // Use your actual 256-bit key
        const iv1 = IV_1; // Use your actual 96-bit IV
        ...
        // First encryption
        aesCtx.auth(authData); // Set AAD before encryption
        const staticPubKey = aesCtx.encrypt(plaintext);
        const tagToEncryptedKey = aesCtx.finish();

        console.log('Static PubKey Encrypted:', staticPubKey.toString('hex'));
        console.log('Tag to Encrypted Key:', tagToEncryptedKey.toString('hex'));

        // Resetting the cipher with the same IV
        // aesCtx.reset(iv1);
        aesCtx.auth(authData);

        // Decrypting the ciphertext
        const decryptedData = aesCtx.decrypt(staticPubKey, tagToEncryptedKey);
        console.log('Decrypted Data:', decryptedData.toString('hex'));
