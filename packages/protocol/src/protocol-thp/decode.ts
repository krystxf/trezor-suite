...
import { TransportProtocolDecode, TransportProtocolState } from '../types';
...
const readProtobufMessage = (
    ...
    return {
        ...
        length: length - (1 + 2 + TAG_LENGTH + CRC_LENGTH), // TODO: messagePayload.length ?
...
export const isExpectedResponse = (bytes: Buffer, protocolState?: TransportProtocolState) => {
    ...
    for (let i = 0; i < expectedResponses.length; i++) {
        if (magic === expectedResponses[i]) {
            // TODO: continuation packet is not masked by controlBit?
