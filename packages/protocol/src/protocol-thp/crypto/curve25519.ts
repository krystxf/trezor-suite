...
const c3 = BigInt('19681161376707505956807079304988542015446066515923890162744021073123829784752'); // sqrt(-1)
const c4 = BigInt('7237005577332262213973186563042994240829374041602535252466099000494570602493'); // (p - 5) // 8
const a24 = BigInt(121666); // (J + 2) // 4
...
function decodeCoordinate(coordinate: Uint8Array): bigint {
    ...
    return littleEndianBytesToBigInt(array) % p;
}
...
export function curve25519(privateKey: Uint8Array, publicKey: Uint8Array): Buffer {
    ...
    const u = decodeCoordinate(publicKey);
    ...
    for (let i = 255; i >= 0; i--) {
        ...
        swap = bit;
        [x2, z2, x3, z3] = ladderOperation(x1, x2, z2, x3, z3);
    }
