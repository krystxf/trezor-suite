...
import { TransportProtocolState } from '../types';
...
// TODO: host should not encode responses from Trezor?
// const createChannelResponsePayload = (
//     data: Record<string, unknown>,
//     protobufEncoder: ProtobufEncoder,
// ) => {
//     const nonce = getBytesFromField(data, 'nonce');
//     if (!nonce) {
//         throw new Error('ThpCreateChannelResponse missing nonce field');
//     }
//     const newChannel = getBytesFromField(data, 'new_channel');
//     if (!newChannel) {
//         throw new Error('ThpCreateChannelResponse missing new_channel field');
//     }
//     const properties = getBytesFromField(data, 'properties');
//     if (!properties) {
//         throw new Error('ThpCreateChannelResponse missing properties field');
//     }

//     const { payload } = protobufEncoder('ThpDeviceProperties', properties);

//     return Buffer.concat([nonce, newChannel!, payload]);
// };

const handshakeInitRequestPayload = (
    data: Record<string, unknown>,
    _protocolState: TransportProtocolState,
) => {
...
const getPayload = (
    ...
    // if (name === 'ThpCreateChannelResponse') {
    //     return createChannelResponsePayload(data, protobufEncoder);
    // }
    ...
    // throw new Error(`Unknown Thp message type ${name}`);
...
export const encodePayload = (
    ...
    const payload = getPayload(name, data, protobufEncoder, protocolState);

    return payload;
};

export const encodeProtobufPayload = () => {};
...
const createChannelResponse = (data: Buffer, channel: Buffer) => {
    const length = Buffer.alloc(2);
    length.writeUInt16BE(data.length + CRC_LENGTH);

    const magic = Buffer.from([THP_CREATE_CHANNEL_RESPONSE]);
    const message = Buffer.concat([magic, channel, length, data]);
    const crc = crc32(message);
...
const handshakeInitResponse = (data: Buffer, channel: Buffer) => {
    const message = Buffer.concat([Buffer.from([THP_HANDSHAKE_INIT_RESPONSE]), channel, data]);
    const crc = crc32(message);
...
const handshakeCompletionResponse = (data: Buffer, channel: Buffer, recvBit: number) => {
    const length = Buffer.alloc(2);
    length.writeUInt16BE(data.length + CRC_LENGTH);

    const magic = addSequenceBit(THP_HANDSHAKE_COMPLETION_RESPONSE, recvBit);
    const message = Buffer.concat([magic, channel, length, data]);
    const crc = crc32(message);
...
const encodeThpMessage = (
    ...
    if (messageType === 'ThpCreateChannelResponse') {
        return createChannelResponse(data, channel);
    }
    ...
    if (messageType === 'ThpHandshakeInitResponse') {
        return handshakeInitResponse(data, channel);
    }
    ...
    if (messageType === 'ThpHandshakeCompletionResponse') {
        return handshakeCompletionResponse(data, channel, protocolState.sendBit || 0);
    }

    // if (messageType === 'ThpReadAck') {
    //     return ack(data, channel, syncBit);
    // }
