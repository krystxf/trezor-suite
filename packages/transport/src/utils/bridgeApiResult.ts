...
export function info(res: UnknownPayload) {
    ...
    const protocolMessage = !!res.protocolMessage;

    return success({ version, configured, protocolMessage });
}
...
export function empty(_res: UnknownPayload) {
    // TODO: this should never happen? reason: json parse error has no fallback anymore
